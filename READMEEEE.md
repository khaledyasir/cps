# READMEEEE.md – Full System Documentation



---
# File: Ouster LiDAR Full Setup and SLAM Guide.md
...
---
# File: SENSOR_OPS_DOCUMENTATION.md
...
---
# Technical Hurdles & Solutions (Semantic Navigation System)

## 1. CUDA Out Of Memory (OOM)
- Problem: SAM ViT-H required ~7GB VRAM; GPU only had 4GB.
- Fix:
  - Switched to SAM ViT-B.
  - Reduced points_per_side.
  - Disabled crop layers.
  - Added torch.cuda.empty_cache().

## 2. Missing CLIP Embeddings
- Problem: SemanticQueryEngine returned random results.
- Fix:
  - Modified OpenVocabularyDetector to return image_features.
  - Updated SemanticMapper to store & average embeddings.

## 3. Regex Parsing Failure
- Problem: Robot didn’t understand “navigate”, “go”, etc.
- Fix:
  - Added flexible regex patterns for verbs & prepositions.

## 4. SAM Hallucination ("Table" from black image)
- Problem: np.zeros() test image produced digital-noise hallucinations.
- Fix:
  - Replaced mock generator with real-image downloader.

## 5. HTTP 403 Forbidden (Image Download)
- Problem: Wikimedia blocked our Python downloads.
- Fix:
  - Added manual urllib.request with User-Agent spoof.

## 6. Module Import Errors
- Problem: Running scripts from src/scripts caused broken imports.
- Fix:
  - Added sys.path.append(...) to include parent project root.
